...
public abstract class Cifrado {
	...
	public static byte[] recibirClaveAESCifradaDelCliente(DataInputStream entrada) {
	    try {
	        // Leer la longitud de la clave AES cifrada
	        int longitudClave = entrada.readInt();
	        // Leer los bytes de la clave AES cifrada
	        byte[] claveAESCifrada = new byte[longitudClave];
	        entrada.readFully(claveAESCifrada);
	        return claveAESCifrada;
	    } catch (Exception e) {
	        e.printStackTrace();
	        return null;
	    }
	...
	public static PublicKey bytesAPublicKey(byte[] bytesPublicKey) {
        try {
            // Obtener las especificaciones de la clave pública RSA
            X509EncodedKeySpec spec = new X509EncodedKeySpec(bytesPublicKey);
            
            // Obtener una instancia de KeyFactory para RSA
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
            
            // Generar la instancia de PublicKey a partir de las especificaciones
	...
	public static byte[] leerDatosDataInputStream(DataInputStream entrada, int longitud) {
	    try {
